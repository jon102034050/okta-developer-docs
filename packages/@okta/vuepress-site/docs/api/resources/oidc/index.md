...
Clients can opt-out of automatic key rotation by changing the client sign-in mode. In this case, passing the `client_id` with your request retrieves the keys for that specific client. Note, passing the `client_id` is only applicable for Okta Org Authorization Server.
